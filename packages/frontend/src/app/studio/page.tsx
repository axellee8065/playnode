...
const contentRows: ContentRow[] = [
  { title: 'Monster Hunter Wilds - Early Weapon Guide', type: 'drop', views: '48.2K', revenue: 820.0, date: '2026-03-28', status: 'published' },
  { title: 'Elden Ring DLC - Complete Boss Walkthrough', type: 'drop', views: '35.1K', revenue: 640.0, date: '2026-03-22', status: 'published' },
  { title: 'Stellar Blade - Honest Review', type: 'review', views: '22.8K', revenue: 310.0, date: '2026-03-15', status: 'published' },
  { title: 'Palworld Multiplayer Setup Guide', type: 'drop', views: '18.5K', revenue: 280.0, date: '2026-03-10', status: 'published' },
  { title: 'GTA VI Pre-Order Link', type: 'shop', views: '12.3K', revenue: 190.0, date: '2026-03-05', status: 'published' },
  { title: 'Hollow Knight: Silksong Review (Draft)', type: 'review', views: '--', revenue: 0, date: '2026-04-02', status: 'draft' },
];
...
const StudioPage: FC = () => {
  ...
  const liveContentRows: ContentRow[] | null =
          ...
          <div className="max-w-[1200px] mx-auto px-4 lg:px-6 py-8 space-y-8">
                ...
                <motion.div key={s.label} variants={fadeUp}>
                  <Card className="h-full">
                    ...
                    <div className="flex items-end gap-2">
                      {'amount' in s ? (
                        <UsdcAmount amount={s.amount!} size={s.size ?? 'md'} showLabel />
                      ) : (
                    ...
                    {weeklyRevenue.map((d, i) => {
                      ...
                      return (
                        <div key={d.day} className="flex-1 flex flex-col items-center gap-2">
                          ...
                          <span className="font-mono text-[10px] text-pn-muted">{d.day}</span>
                        </div>
                      );
                    })}
                  </div>
                      ...
                      <div key={src.name}>
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-[10px] text-pn-muted">{src.pct}%</span>
                          </div>
                        </div>
                        <div className="h-2 bg-pn-surface-2 rounded-sm overflow-hidden">
                          <AnimatedBar pct={src.pct} colorClass={src.color} delay={0.4 + i * 0.08} />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            >
              <Card className="!p-0 overflow-hidden">
                <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-pn-border">
                  <p className="font-mono text-[10px] uppercase tracking-wider text-pn-muted">
                    Recent Content
                  </p>
                  <Link
                    href="/studio/content"
                    className="text-xs font-mono text-pn-green hover:text-pn-green/80 transition-colors"
                  >
                    View All
                  </Link>
                </div>
